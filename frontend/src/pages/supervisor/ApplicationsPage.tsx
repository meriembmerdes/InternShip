...
try{setMessage('');setError('');await applicationService.updateStatus(id,status);setMessage(status===('ACCEPTEE' as ApplicationStatus)?'Candidature acceptée.':'Candidature refusée.');await loadApplications();}
catch(error){console.error(error);setError('Impossible de modifier la candidature.');}
};

const statusLabel=(status:ApplicationStatus)=>{
if(status===('EN_ATTENTE' as ApplicationStatus))return'En attente';
if(status===('ACCEPTEE' as ApplicationStatus))return'Acceptée';
if(status===('REFUSEE' as ApplicationStatus))return'Refusée';
return'Annulée';
};

const statusClass=(status:ApplicationStatus)=>{
if(status===('EN_ATTENTE' as ApplicationStatus))return'bg-yellow-100 text-yellow-700';
if(status===('ACCEPTEE' as ApplicationStatus))return'bg-green-100 text-green-700';
if(status===('REFUSEE' as ApplicationStatus))return'bg-red-100 text-red-700';
...
<p className="mt-2 text-gray-500">Gérez les candidatures des étudiants à vos offres.</p>
</div>
...
<div><p className="text-sm text-gray-500">Étudiant</p><p className="font-medium">{application.student?.firstName||application.student?.lastName?`${application.student?.firstName||''} ${application.student?.lastName||''}`.trim():'Non renseigné'}</p></div>
<div><p className="text-sm text-gray-500">Email</p><p className="font-medium">Non renseigné</p></div>
</div>
...
{application.status===('EN_ATTENTE' as ApplicationStatus)&&<div className="mt-6 flex gap-3">
<button onClick={()=>updateStatus(application.id,'ACCEPTEE' as ApplicationStatus)} className="rounded-lg bg-green-600 px-5 py-2 text-white hover:bg-green-700">Accepter</button>
<button onClick={()=>updateStatus(application.id,'REFUSEE' as ApplicationStatus)} className="rounded-lg bg-red-600 px-5 py-2 text-white hover:bg-red-700">Refuser</button>
</div>}
</div>
))}
</div>}
